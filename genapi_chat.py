"""Запрос к GenAPI через OpenAI-совместимый proxy (HTTP requests)."""
...
def main() -> None:
    ...
    if not api_key:
        print("Ошибка: не найден GENAPI_KEY в .env файле")
        return

    base_url = os.getenv("GENAPI_BASE_URL", "https://proxy.gen-api.ru/v1").rstrip("/")
    model = os.getenv("GENAPI_MODEL", "").strip()
    if not model:
        print("Ошибка: укажите GENAPI_MODEL в .env (id модели с gen-api.ru)")
        return
